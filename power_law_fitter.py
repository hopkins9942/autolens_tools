...
Command line arguments:
    1: path to directory containing datasets, e.g. /etc/autolens_tools/ExampleData/NFW_sersic
    2: name of dataset, e.g. (0.0, 0.0)_13.0_1.0_(0.1, 0.0)_(0.0, 0.0)_0.2
    3: radius of mask to use when fitting.
...
lens_model_1 = al.GalaxyModel(redshift=0.5, mass=al.mp.EllipticalPowerLaw)
...
phase_1_result = phase_1.run(
    dataset=imaging, mask=mask, info=info, pickle_files=pickle_files
)
...
aplt.FitImaging.subplot_fit_imaging(
    fit=phase_1_result.max_log_likelihood_fit, sub_plotter=phase_1_sub_plotter
)
...
lens_model_2 = al.GalaxyModel(
    redshift=0.5, mass=af.PriorModel(al.mp.EllipticalPowerLaw),
)
...
phase_2_result = phase_2.run(
    dataset=imaging, mask=mask, info=info, pickle_files=pickle_files
)

phase_2_sub_plotter = aplt.SubPlotter(
    output=aplt.Output(path=output_path, filename="Phase2", format="png")
)
aplt.FitImaging.subplot_fit_imaging(
    fit=phase_2_result.max_log_likelihood_fit, sub_plotter=phase_2_sub_plotter
)
